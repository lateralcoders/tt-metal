...
void DispatchMemMap::reset(const CoreType& core_type, const uint32_t num_hw_cqs) {
    ...
    for (auto dev_addr_idx = 0; dev_addr_idx < num_dev_cq_addrs; dev_addr_idx++) {
        ...
        if (dev_addr_type == CommandQueueDeviceAddrType::PREFETCH_Q_RD) {
            ...
            static constexpr uint32_t k_RbEntries = 32;
            device_cq_addr_sizes_[dev_addr_idx] =
                k_RbEntries * std::max(tt_fabric::PUSH_CLIENT_INTERFACE_SIZE, tt_fabric::PULL_CLIENT_INTERFACE_SIZE);
            ...
            // static constexpr uint32_t k_RbEntries = 64;
            // device_cq_addr_sizes_[dev_addr_idx] = tt_fabric::PULL_CLIENT_INTERFACE_SIZE * k_RbEntries;
            // } else {
    ...
    for (auto dev_addr_idx = 1; dev_addr_idx < num_dev_cq_addrs; dev_addr_idx++) {
        ...
        std::cout << fmt::format("{} = {:#x}\n", magic_enum::enum_name(dev_addr_type), device_cq_addrs_[dev_addr_idx]);
    }
