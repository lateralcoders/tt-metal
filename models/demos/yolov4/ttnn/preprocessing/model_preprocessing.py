...
from models.demos.yolov4.ttnn.preprocessing.overrides import (
    Downsample1ConvArgsOverride,
    apply_overrides,
)
...
def _create_ds1_model_parameters(conv_args, resolution):
    if resolution == (320, 320):
        json_file = "models/demos/yolov4/resources/ds1_conv_args_overrides_320_320.json"
        with open(json_file, "r") as f:
            conv_args_overrides = Downsample1ConvArgsOverride.from_json(json.load(f))

        apply_overrides(conv_args, conv_args_overrides)
    elif resolution == (640, 640):
        json_file = "models/demos/yolov4/resources/ds1_conv_args_overrides_640_640.json"
        with open(json_file, "r") as f:
            conv_args_overrides = Downsample1ConvArgsOverride.from_json(json.load(f))
