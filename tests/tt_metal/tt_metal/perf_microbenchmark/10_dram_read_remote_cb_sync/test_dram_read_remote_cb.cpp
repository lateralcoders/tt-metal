...
bool validation_bfp8_b(
    ...
    result_untilized = untilize(result_bfp8, kt * 32 / num_blocks * cb_num_blocks, nt * 32);
...
bool validation_fp16(
    ...
    auto result_untilized = untilize(result_flat_layout, kt * 32 / num_blocks * cb_num_blocks, nt * 32);
...
bool validation_mixed_df(
    ...
    auto values_fp16 = tilize(input_tensor_fp16.get_values(), kt * 32, nt * 32);
        ...
        if (tile_format == tt::DataFormat::Bfp8_b) {
            for (uint32_t i = 0; i < num_mixed_df_layers; ++i) {
                if (i % 2 == 0) {  // even layers
                    auto input_vec_tilized = tilize(tensor_fp8.get_values(), k, n);
                    ...
                    auto input_vec_tilized = tilize(tensor_fp16.get_values(), k, n);
