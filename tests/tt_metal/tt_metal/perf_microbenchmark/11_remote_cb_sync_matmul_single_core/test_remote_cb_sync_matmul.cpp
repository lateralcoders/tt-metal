...
bool validation_bfp8_b(
    ...
    result_untilized = untilize(result_bfp8, mt * 32, nt * 32);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (tile_format == tt::DataFormat::Bfp8_b) {
            // in1 DRAM
            for (uint32_t i = 0; i < num_layers; ++i) {
                auto input_vec_tilized = tilize(in1_tensor_fp8.get_values(), k, n);
            ...
            // in0
            auto activations_tilized = tilize(in0_tensor_fp8.get_values(), m, k * num_receivers);
            ...
            // in1
            for (uint32_t i = 0; i < num_layers; ++i) {
                auto input_vec_tilized = tilize(in1_tensor_fp16.get_values(), k, n);
            ...
            // in0
            auto activations_tilized = tilize(in0_tensor_fp16.get_values(), m, k * num_receivers);
