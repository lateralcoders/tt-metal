...
def test_quant_dequant_per_tensor_1d(device, x0, input_dtype, scale_dim, zero_point_dim):
    ...
    scale, zero_point = calculate_scale_zero_point(input_tr, -128, 127)
...
def test_requant_per_tensor_1d(device, x0, input_dtype, scale_dim, zero_point_dim, scale_r_dim, zero_point_r_dim):
    ...
    input_tr = torch.rand(x0, dtype=torch.float32)
    scale, zero_point = calculate_scale_zero_point(input_tr, -128, 127)
    scale_r, zero_point_r = calculate_scale_zero_point(input_tr, -37, 73)
...
def test_quantization_program_cache(device, use_program_cache, input_dtype):
    ...
    for dim in [1, 2, 3, 4]:
        for i in range(5):
            ...
            scale_r, zero_point_r = calculate_scale_zero_point(torch_input_tensor, -50 - i, 42 + i)
            ...
            requantized_tensor = ttnn.requantize(quantized_tensor, scale, zero_point, scale_r, zero_point_r)
            dequantized_tensor = ttnn.dequantize(requantized_tensor, scale_r, zero_point_r, dtype=input_dtype)
